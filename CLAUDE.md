...
그러나 worktree 를 늘려도 넘지 못하는 벽이 있다 (2026-08 기준 측정):

```
디스크 여유   5.6 GB      저장소 5.9 GB (.git 1.1 + build 3.3 + binary 0.5)
RAM          16 GB       Gradle/Kotlin 데몬이 이미 5개
코어          8
```

worktree 하나를 추가하면 체크아웃 ~1.5 GB 에 빌드 산출물 2.5 GB 이상이 더 붙는다.

| | 동시 실행 상한 |
|---|---|
| 빌드·테스트를 도는 에이전트 | **2** (worktree 격리) |
| 읽기 · 분석 · 문서만 하는 에이전트 | 3~4 추가 가능 |
