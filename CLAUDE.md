...
**두 수단을 다 채운다.** 예전에는 `Agent` 도구를 명시적 요청 없이 쓰지 못하게 되어 있었는데, 그 규칙은
2번 계정이 살아 있고 조율 세션의 한도를 아껴야 하던 때의 것이다. 2번 계정이 없어진 지금 그 제약을
유지하면 실행 수단이 `agy` 하나로 줄어 동시 실행이 막힌다.

    기본값   agy -p "..." --model ...        (배경 프로세스)
    기본값   Agent(model=..., run_in_background: true)

**둘의 성질이 다르므로 성질에 맞춰 나눈다:**

| | `agy` | `Agent` 도구 |
|---|---|---|
| 인터럽트 내성 | 배경 프로세스라 조율 세션이 끊겨도 산다 | 알림이 조율 세션으로 돌아온다 |
| 오래 걸리고 폴링이 필요한 작업 | 헤드리스라 **깨어날 다음 턴이 없다** | **이쪽에 준다** (기기 빌드, 에뮬레이터) |
| 모델 선택 | Gemini · Claude · GPT-OSS 계열 | `model` 인자 (`opus`/`sonnet`/`haiku`) |
| 한도 | 모델 계열별 별도 집계 | 조율 세션과 같은 계정 |

**`Agent` 도구는 `model` 을 안 주면 조율 세션의 모델을 상속해 전부 opus 가 된다.** 아래
"조율 세션의 서브 에이전트도 등급을 나눠 쓴다" 절의 등급표를 반드시 적용한다.
...
| 작업 성격 | 권장 모델 |
|---|---|
| 툴체인 배선, 설계 전제를 건드리는 작업 | `Agent` 도구 `opus`, 또는 `agy` 의 `claude-opus-4-6-thinking` |
...
`claude -p` 로 띄운 2번 계정에서 세 번 겪었고, `agy` 도 같다 — 오늘 tempdir 작업이 같은 실패였다. 세 번째는 `xcodebuild` 를 배경으로 띄우고
...
**기기 빌드처럼 오래 걸리고 폴링이 필요한 작업은 `agy` 대신 조율 세션의 `Agent` 도구에게 준다.**
