...
### 1번 계정(조율 세션)의 `Agent` 도구로 서브 에이전트를 만들지 않는다

**명시적 요청이 없으면 `Agent` 도구를 쓰지 않는다.** 작업은 `agy` 와 2번 계정(`~/.claude-alt`)에
배분한다. 1번 계정의 한도는 조율·검증·커밋·머지에 쓰는 것이고, 서브 에이전트를 그 계정으로
띄우면 조율에 쓸 한도를 구현이 먹는다.

    기본값   agy -p "..." --model ...            (배경 프로세스)
    기본값   CLAUDE_CONFIG_DIR=~/.claude-alt claude -p "..."   (배경 프로세스, 환경변수 정리 필수)
    예외     사용자가 "서브 에이전트로" 라고 명시했을 때만 `Agent` 도구

이미 `Agent` 도구로 띄워 돌고 있는 것은 죽이지 않는다 — 끝내고 결과를 착지시킨 뒤, 새 배분부터
위 규칙을 따른다. 아래 "조율 세션의 서브 에이전트도 등급을 나눠 쓴다" 절의 등급표는 그 예외의
경우에만 적용된다.
