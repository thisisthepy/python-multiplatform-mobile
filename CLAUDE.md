...
2026-08-24, 일주일치 작업을 되짚어보니 **개별 단계는 대부분 타당했는데 총합이 지시에서 멀어져
있었다.** 그 표류가 어떻게 일어났는지가 아래 다섯 가지이고, 각각에 그것을 잡아낼 검사가 붙어 있다.
반성문이 아니라 검사 목록으로 읽어라.

#### 5.1 사양은 문서가 아니라 사용자가 준 것이다

`UI.ipynb` 가 이 프로젝트의 사용자 대상 사양인데, **"추적 안 되는 사용자 파일이니 건드리지 마라"로
분류하고 에이전트 프롬프트에도 그렇게 써 보냈다.** 일주일간 한 번도 읽지 않았다. 대신 ROADMAP 과
`docs/` 를 사양처럼 다뤘는데, 그것들은 **작업의 산출물이지 요구사항이 아니다.**

    작업 시작 전:  사용자가 직접 만든 파일(노트북, 예시 프로젝트, `(플러그인예시)build.gradle.kts`)을
                   먼저 찾아 읽는다. 없으면 "사양이 어디 있는지" 를 묻는다.
    ROADMAP 은 진척 기록이지 사양이 아니다. 둘이 어긋나면 **사양이 이긴다.**

#### 5.2 사용자가 구조를 말하면 그것이 제약이지 제안이 아니다

*"pythonx 에는 실제 패키지가 존재해야 하고, 그 코드가 androidx 모듈을 가져와 파이써닉하게 쓰도록
구조를 수정한다"* 는 지시를 받고도, **어댑터가 `pythonx.*` 를 합성하는 구조**를 만들었다.
그 구조는 모든 합성 모듈에 `__path__ = []` 를 박아서 **디스크의 실제 패키지 파일이 로드될 수 없게**
만든다. 즉 지시받은 구조를 물리적으로 불가능하게 만들어 놓고, 그 위에서 "어댑터가 다 해주니
래퍼는 필요 없다"며 그 패키지의 코드를 지웠다.

    설계를 고르기 전에: 사용자가 말한 구조와 내 설계가 **양립하는지** 명시적으로 확인한다.
    "내 방식이 더 낫다" 는 판단이 들면 **그것을 구현하지 말고 말한다.**

#### 5.3 테스트 수는 진척이 아니다

매 회차 "레포별 착지 완료" 로 보고했는데, 그 안에는 **구현·결함 수정·문서 정정·삭제**가 같은
칸에 섞여 있었다. `pythonx-compose` 의 71개 테스트는 전부 *부재*(죽은 토큰이 없다, 독스트링이
있다)를 확인하는 것이었고 **기능은 하나도 늘지 않았다.** 숫자가 올라가니 진척처럼 보였다.

    보고할 때 나눈다:  기능 추가 / 결함 수정 / 테스트 추가 / 문서 정정 / 삭제
    "미구현 없음" 이라고 쓰기 전에 **무엇을 기준으로 센 것인지** 밝힌다.
    ROADMAP 을 다 닫아도 `docs/ecosystem.md` §5 의 Gap 은 그대로일 수 있다 — 실제로 그랬다.

#### 5.4 내가 쓴 기준이 답을 정해버린다

"지워진 코드가 이관된 것인지" 감사하는 프롬프트에 **"식별자가 다른 레포에서 발견돼야 이관"** 이라고
기준을 써 넣었다. `jclass` 파이썬 코드가 Kotlin 스캐너로 다시 구현되면 식별자가 같을 리 없으므로,
**그 기준은 이관을 구조적으로 못 잡는다.** 감사는 "이관 없음" 을 냈고 나는 그것을 사실로 보고했다.

    감사·조사 프롬프트를 쓸 때: **이 기준으로 찾을 수 없는 것이 무엇인지** 한 줄 적는다.
    에이전트 결론이 내 가설과 일치하면 **그때 더 의심한다.** 내가 기준을 줬기 때문이다.
    능력 기준으로 물어라 — "이 파일이 주던 것을 지금은 무엇이 주는가".

#### 5.5 실패할 수 없는 검증은 검증이 아니다

같은 주에 세 번 나왔다.

- 픽스처 세 모듈을 **한 Gradle 명령**으로 돌려 초록을 봤다. 따로 돌리면 `artifact` 가 24개 실패였다.
  순서 의존성을 검증 방식이 가려주고 있었다. → **모듈마다 따로 돌린다.**
- `git status --short` 가 **22줄**인데 에이전트는 21개 파일을 바꿨다고 보고했다. 묻지 않고
  `git add -A` 했다. 그 한 줄이 **탬퍼 스텁으로 바뀐 프로덕션 파일**이었고 그대로 커밋됐다.
  → **`git add -A` 를 쓰지 마라. 파일을 명시하거나, 개수가 보고와 다르면 멈춘다.**
- 21개 픽스처가 공개 API 대신 **날 주소를 직접 쓰고 있어서**, 그 API 를 통째로 비워도 통과했다.
  → 새 공개 경로를 만들면 **그것을 무력화했을 때 실제로 깨지는지** 확인한다. 안 깨지면 아무도 안 쓰는 것이다.

#### 5.6 하위 호환은 규칙보다 아래다

*"바인더에는 androidx 를 pythonx 로 바꾸는 기능이 있으면 안 된다"* 는 규칙을 받고, 에이전트에게
**"기본값은 지금 값으로 두어라, 기존 호출자가 깨지면 안 된다"** 고 지시했다. 그 기본값이 곧
그 규칙이 금지한 것이다. 호출자를 지키려다 **규칙을 뒤집었다.**

    규칙과 하위 호환이 부딪히면 **규칙이 먼저다.** 깨지는 호출자는 목록으로 만들어 고친다.
    "안 깨지게 하려면 X 를 남겨야 한다" 는 생각이 들면, X 가 금지된 그것인지 먼저 본다.

#### 5.7 결론을 실행에 옮기기 전에 보여준다

디스크 159 GB 삭제, 3일 묵은 프로세스 종료, 두 레포에 걸친 API 변경 배분 — 전부 **결정하고 나서
보고**했다. 각각은 근거가 있었지만 공통점은 **사용자가 아직 보지 못한 결론을 내가 실행했다**는 것이다.

    즉시 요청 범위를 넘는 것(다른 레포, API 시그니처, 파일 삭제, 프로세스 종료, 원격 푸시)은
